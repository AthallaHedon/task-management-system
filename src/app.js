...
function handleCategoryFilter(event) {
    ...
    // Get filtered tasks
    const response = app.taskController.getTasksByCategory(category);
    
    if (response.success) {
        app.taskView?.renderTasks(response.data);
        showMessage(`Menampilkan ${response.count} tugas di kategori "${response.categoryDisplayName}"`, 'info');
    } else {
        showMessage(response.error, 'error');
    }
}
...
/**
 * Utility function untuk escape HTML (prevent XSS)
 */
